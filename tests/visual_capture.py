...
with sync_playwright() as p:
    ...
    required = ["Gestão de Pagamentos", "Revisões dos Técnicos"]
    ...
    (OUT / "visual-check.txt").write_text("OK" if not missing else "FALTANDO: " + ", ".join(missing), encoding="utf-8")
    browser.close()

if missing:
    raise AssertionError("Elementos obrigatorios ausentes: " + ", ".join(missing))
if errors:
